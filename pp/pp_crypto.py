...
SOFTWARE = 'rsa_openssl_0.9.7c'

N_SET = 64 # number of cache set
REPEAT_NUM = 16
SETTING = 'pp_dcache_%d' % REPEAT_NUM
...
length_dict = {
    'rsa_openssl_3.0.0': 256 * 4,
    'rsa_mbedtls_3.0.0': 256 * 4,
    'rsa_libgcrypt_1.9.4': 256 * 4,
    'aes_openssl_3.0.0': 128,
}

PAD_LEN = length_dict[SOFTWARE] * REPEAT_NUM
...
key_dir = config.openssl_key_dir
...
for sub in sub_list:
    for repeat_idx in SUBNUM_DICT[sub]:
        ...
        total_key_list = sorted(os.listdir(key_dir + sub))
        unit_len = int(len(total_key_list) // total_num)

        if seg_id == total_num - 1:
            key_list = total_key_list[seg_id*unit_len:]
        else:
            key_list = total_key_list[seg_id*unit_len:(seg_id+1)*unit_len]

        for i, key_name in enumerate(tqdm(key_list)):

            key_path = os.path.join(key_dir, sub, key_name)
            prefix = key_name.split('.')[0].replace('key_', '')
            suffix = '.npz'
            npz_path = os.path.join(npz_dir, str(repeat_idx) + '_' + sub, prefix + suffix)

            command = '%s %s' % (os.path.join(config.lib_dir, SOFTWARE, 'a.out'), key_path)
